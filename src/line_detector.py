...
class LineDetector:
    ...
    def image_callback_other(self, image_msg):
        """ """
        ...
        for index, pixel in enumerate(mask[y_pixel]):
            if pixel == 255:
                found_x_vals.append(index)

        if found_x_vals == []:
            x_pixel = math.ceil(0.5 * width)
        else:
            x_pixel = np.average(found_x_vals)
    ...
    def image_callback(self, image_msg):
        ...
        im2, contours, _ = cv.findContours(
